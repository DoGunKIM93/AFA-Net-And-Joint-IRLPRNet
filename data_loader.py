...
class Dataset(torchDataset):
    ...
    def _popItemInGlobalFileListByIndex(self, index):
        ...
        if datasetComponentType != "ImageSequence":
            ...
            dFP = (
                Config.param.data.path.datasetPath + dFP
                if isinstance(dFP, str)
                else [Config.param.data.path.datasetPath + x for x in dFP]
            )
            ...
            lFP = (
                (
                    Config.param.data.path.datasetPath + lFP
                    if isinstance(lFP, str)
                    else [Config.param.data.path.datasetPath + x for x in lFP]
                )
                if lFP is not None
                else None
            )
...
class DatasetHighball(torchDataset):
    def __init__(
        self,
        datasetComponentParamList: List,
        batchSize: int,
