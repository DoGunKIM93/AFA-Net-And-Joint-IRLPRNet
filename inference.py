...
def _loadModel(inferencePresetName):
    ...
    model = module(*list(( x if x.replace('.','',1).isdigit() is False else ( int(x) if x.find('.') is -1 else float(x) ) ) for x in paramList))
    model.cuda()
    ...
    model.eval()
    print(f"{Config.inferenceDict[inferencePresetName]['model']} Loaded with {mthd} mode." if mthd != 'FAILED' else f"{Config.inferenceDict[inferencePresetName]['model']} Load Failed.")
    return model
...
def inferenceSingle(inp, inferencePresetName, outputType = None, outputPath = None):
    ...
    assert outputType in [None, 'FILE', 'TENSOR', 'NPARRAY', 'PIL'], f"inference.py :: outputType '{outputType}' is not supported. Supported types: 'FILE', 'TENSOR', 'NPARRAY', 'PIL'"
    assert outputType != 'FILE' or (outputType == 'FILE' and outputPath is not None), "inference.py :: outputPath must be exist if outputType is 'FILE'"
    ...
    #convert to output
    if outputType == 'FILE': 
        [utils.saveImageTensorToFile({'Result':x},f'{outputPath.split(".")[0]}-{i}.{outputPath.split(".")[1]}',caption=False) for i, x in enumerate(out)] if isinstance(out, tuple) or isinstance(out, list) else utils.saveImageTensorToFile({'Result':out},outputPath,caption=False)
        utils.saveImageTensorToFile({'Result':inp},f'{outputPath.split(".")[0]}-original.{outputPath.split(".")[1]}',caption=False)
        print("Saved.")
        return
    else:
...
inferenceSingle(args.inputPath, args.inferencePresetName, outputType = 'FILE', outputPath = args.outputPath)
