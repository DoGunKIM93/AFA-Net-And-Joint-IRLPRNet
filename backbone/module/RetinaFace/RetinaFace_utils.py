...
def selectRoi(loc, conf, destination_shape):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    confidence_threshold = 0.85
    nms_threshold = 0.6
    variance = [0.1, 0.2]

    scale = torch.Tensor([destination_shape[3], destination_shape[2], destination_shape[3], destination_shape[2]]).to(device)
    priorbox = PriorBox(image_size=(destination_shape[2], destination_shape[3]))
